...
- **プロキシ API**: `GET /proxy?url=<encoded>` で外部画像を R2 キャッシュ付きでプロキシ配信（許可ホストのみ）
...
functions/[[route]].ts
  GET  /proxy         ← 外部 URL をプロキシ配信（R2 キャッシュ付き、許可ホストのみ）
...
- スキーム制限: `https:` のみ受け付け（`GET /proxy`）
- ホスト許可リスト: プロキシ API は Discord・Twitter・Pixiv・GitHub など許可済みホストのみ受け付け（それ以外は `blocked.png` を返す）
